...
assert.equal(defaultSettings.agentMaxRunMs, 480000);
...
assert.equal(defaultSettings.maintenanceExecutionMode, "strict");
assert.equal(defaultSettings.parallelVectorQueryConcurrency, 3);
assert.equal(defaultSettings.parallelNeighborQueryConcurrency, 3);
assert.equal(defaultSettings.parallelLlmConcurrency, 2);
assert.equal(defaultSettings.backgroundMaintenanceMaxRetries, 2);
assert.equal(defaultSettings.backgroundMaintenanceRetryBaseMs, 800);
assert.equal(defaultSettings.backgroundMaintenanceMaxQueueItems, 24);
