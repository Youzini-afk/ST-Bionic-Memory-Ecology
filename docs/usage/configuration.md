...
- **工作流（默认）**：按现有开关、频率和参数运行完整的召回、提取与维护流程。
- **Agent**：生成前由 Recall Agent 判断是否需要继续深入查询；回复后由后台 Memory Steward 判断本批需要哪些已启用能力，再调用同一套工作流事务。

两种模式始终使用当前聊天的同一份图谱、楼层回退、Luker / Authority 主存储和 Cloud Sync 副本。切换模式不会迁移或复制记忆，既有工作流控制不会被隐藏或废弃。Agent 会在每条新 AI 回复后判断是否需要处理，因此“每 N 条回复”“惊奇触发”和“自动提取晚一楼”只负责工作流模式的调度；其他能力开关和参数继续作为 Agent 可调用流程的权限与边界。

Agent 使用 BME 自己的独立记忆 LLM 配置，不读取 DOA 的模型。Agent 模式需要在 BME 中填写支持工具调用的 URL 与模型；未配置或调用失败时会安全回退到完整工作流。默认上下文窗口为 `128000` tokens；上下文接近窗口时自动总结。防失控边界默认是单任务 `500` 次工具调用和 `8` 分钟，均可在 API 配置中调整。
...
- 反思。
- ENA Planner 规划。
- Agent 模式的 Recall Agent 与后台 Memory Steward。
...
- reroll 不会重新运行 ENA；它复用父 user 楼层的持久召回。
...
- 如果你要对很早的楼层做“重新提取范围”或事务回滚，建议临时关闭渲染限制或调大数量并刷新。
- 当 ST-BME 检测到当前 `context.chat` 很可能只是最近 N 条渲染切片时，会暂停破坏性回滚，避免误清空运行时图谱。
