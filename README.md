...
4. **选择运行方式** — 默认“工作流”保留完整既有行为；切到“Agent”后，BME 模型会在同一份聊天记忆上决定召回深度和本轮需要的已启用能力。Agent 需要在 BME 中独立配置支持工具调用的 URL 与模型，不读取 DOA 模型。（若未配置或配置失败，系统会自动回退到工作流模式。）
5. **配置 Embedding** — 默认使用直连 Embedding API，可连接独立的外部 embedding 服务；也可切换到 SillyTavern 后端索引，但仅能使用宿主支持的向量来源。直连时需确保服务允许浏览器跨域访问。
6. **开始聊天** — 正常对话即可，AI 回复后自动处理记忆，下次生成前自动召回。
7. **查看结果** — "总览"看状态，"任务 → 记忆浏览"看节点，图谱区域看关系网络，用户消息下方可能出现召回卡片。
...
- **楼层回滚依赖可逆日志** — 日志覆盖不足时会保留安全检查点并停止，不会因删楼自动全量重建或调用 LLM。
