...
export default {
  ...
  "panel.agent.modelHelp": "BME Agent 独立使用这里配置的模型，不会借用当前聊天模型或 DOA 的 Agent 模型。",
  "panel.agent.providerHelp": "Memory Steward、召回 Agent 与 ENA 共享这套 BME 专用连接。支持 OpenAI 兼容渠道、Anthropic Claude、Google AI Studio / Gemini。",
  "panel.agent.guardTitle": "Agent 运行边界",
  "panel.agent.guardHelp": "上下文按 token 自动压缩；工具次数与运行时间只用于阻止后台任务失控。",
  "panel.agent.contextWindow": "上下文窗口（tokens）",
  "panel.agent.maxTools": "后台任务工具调用上限",
  "panel.agent.maxMinutes": "后台任务运行上限（分钟）",
  ...
  "llm.providerHelp.auto": "BME Agent 需要独立配置 API 地址和模型，不会复用当前聊天模型或 DOA 模型。支持 OpenAI 兼容渠道、Anthropic Claude、Google AI Studio / Gemini。",
